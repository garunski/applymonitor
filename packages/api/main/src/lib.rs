...
use endpoints::{health, jobs, root, test};
...
async fn main(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    router
        ...
        .options("/", |_, _| Response::ok("")?.with_cors(&get_cors()))
        .get_async("/health", health::handler)
        .options("/health", |_, _| Response::ok("")?.with_cors(&get_cors()))
        .get_async("/test", test::handler)
        .options("/test", |_, _| Response::ok("")?.with_cors(&get_cors()))
        .get_async(
            "/jobs",
        ...
        .options("/jobs", |_, _| Response::ok("")?.with_cors(&get_cors()))
        .options("/jobs/:id", |_, _| Response::ok("")?.with_cors(&get_cors()))
        .run(req, env)
        .await
}
